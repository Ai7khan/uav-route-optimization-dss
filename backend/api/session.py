"""Live mission session: ties simulation + ML forecast + adaptive planner together
and produces compact state payloads for the operator UI."""
...
from backend.config import GRID, TIME_STEP_MIN, UAV_CRUISE_KMH
from backend.ml import ad_predictor
from backend.ml.forecast import forecast_weather
from backend.optimizer.planner import AdaptivePlanner, plan_once
from backend.optimizer.route import ALT_PRESETS
...
class Mission:
    def __init__(self, scenario_id: str, start: tuple[float, float],
                 goal: tuple[float, float], weights: Weights, alt_m: float = 500.0,
        ...
        self.start = GRID.latlon_to_cell(*start)
        self.goal = GRID.latlon_to_cell(*goal)
        self.weights = weights
        self.alt_m = alt_m
        self.use_forecast = use_forecast

        # Emission history for the AD-activity predictor.
    ...
    def _build_field(self) -> CostField:
        ...
        return CostField(sites, self.scenario.tick, self.alt_m, weather)

    def _effective_sites(self):
        """Sites treated as threats: currently-active, plus those predicted to
        activate soon (pre-emptive avoidance via the AD predictor)."""
        sites = []
        ...
        for j, s in enumerate(self.scenario.ad_sites):
            ...
            if not active and self.use_forecast and t >= 3:
                p = ad_predictor.predict_activity(self.emission_history, t, j)
                active = p >= 0.5
            s2 = s.model_copy()
            s2.active = active
            sites.append(s2)
    ...
    def _record_emissions(self):
        ...
        if self.emission_history.shape[0] == 1 and self.emission_history.sum() == 0 and self.scenario.tick == 0:
    ...
    def _alts(self) -> list[Route]:
        ...
        for label, w in ALT_PRESETS.items():
            r = plan_once(self.field, self.start, self.goal, w, label=label)
            if r:
                out.append(r)
        return out
    ...
    def replan(self, weights: Weights | None = None) -> dict:
        if weights is not None:
            ...
            self.planner.ws, self.planner.wt, self.planner.wf = (
                weights.safety, weights.time, weights.fuel)
        self.route = plan_once(self.field, self.start, self.goal, self.weights)
        self.alternatives = self._alts()
        return self.state()
    ...
    def _uav_position(self):
        if not self.route or not self.route.segments:
            return {"lat": self.start_latlon[0], "lon": self.start_latlon[1]}
        ...
        return {"lat": segs[-1].lat, "lon": segs[-1].lon}
